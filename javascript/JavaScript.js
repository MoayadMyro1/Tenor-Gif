...
btn.addEventListener("click", function (event) {
    ...
    http.onreadystatechange = function () {
        if (http.readyState === 4 && http.status === 200) {
            ...
            for (let i = 0; i < 8; i++)
            {
                ...
                arr[i] = arr[i][0]['gif'];
                arr[i] = arr[i]['url'];
                content.innerHTML += "<div class=" + 'col-3' + "><img src=" + arr[i] + " class=" + 'imgx' + " /></div>";
    ...
    http.open("GET", "https://api.tenor.com/v1/search?q=" + result.innerText +"&key=LIVDSRZULELA&limit=8", true);
    http.send(); 
